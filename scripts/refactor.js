...
for (var d in directories) {
    ...
    for (var i in files) {
        ...
        if (fileName.startsWith("_gallery.")) {
            ...
            const resourceType = newFileName.substr(0,newFileName.indexOf('-'));
            const galleryName = newFileName.substr(newFileName.indexOf('-')+1);
            const newFilePath = newlocation.concat(resourceType);
            const fullFilePath = newFilePath.concat("\\", galleryName);
            const content = JSON.stringify(newData, null, "\t");
            fs.mkdir(newFilePath, { recursive: true }, (err) => { if (err) throw err; });
